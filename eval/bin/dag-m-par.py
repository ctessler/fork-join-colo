...
DESCR='''
The calculation of minimum number of cores per task. Designed for
general DAG tasks, this is the basis of the DAG-OT work.
'''

def mp_job(q, task_path, task, cores_min, cores_max, force):
    try:
        results = dagm.results(task, cores_min, cores_max)
...
def module_start(path, cores_min, cores_max, force=False):
    ...
    logging.info('Calculating minimum cores without co-location ...');
