...
from dagot.LeastPenalty import least_penalty
...
DESCR='''
The calculation of minimum number of cores per task using the Least
Penalty heuristic. Designed for general DAG tasks.
'''
...
def module_start(task_path, cores_min, cores_max, force=False):
    ...
    log_pfx=f'{pfx}(Task:{task_read.name})'
    ...
    cores = res[f'{pfx}-Cores']
    logging.debug(f'{log_pfx} cores required : {cores}')
    return 0
